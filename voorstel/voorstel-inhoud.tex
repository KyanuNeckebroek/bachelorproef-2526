...
toegepast op 300 Mastodon-berichten.
...
\begin{itemize}
    ...
    \item Minimaal 300 Mastodon-berichten, verzameld via de Mastodon API van publieke instances.
\end{itemize}
...
Het onderzoek wordt uitgevoerd in sprints van twee weken. De focus ligt hierbij op een cyclisch proces van \textit{error analysis} en modelverbetering, waarbij de evaluatie van de vorige sprint de koers van de volgende sprint bepaalt:

\begin{itemize}
    \item \textbf{Evaluatie en foutenanalyse:} Na elke sprint worden de resultaten op de validatieset geanalyseerd. Er wordt specifiek gekeken naar patronen in foutieve voorspellingen (bijv. presteert het model consistent slecht op korte berichten of op specifieke instances?).
    \item \textbf{Gerichte optimalisatie:} In plaats van telkens een volledig nieuw model te kiezen, wordt het huidige model (of de dataset) verbeterd op basis van de gevonden fouten. Dit omvat:
    \begin{itemize}
        \item Het verfijnen van preprocessing (bijv. betere afhandeling van Mastodon-specifieke hashtags of emoticons).
        \item Data-augmentatie voor categorieën waar het model moeite mee heeft.
        \item Hyperparameter-tuning (zoals de learning rate of weight decay bij BERT) om gevonden overfitting te reduceren.
    \end{itemize}
    \item \textbf{Modelvergelijking:} In de eerste fases ligt de focus op de traditionele classifiers om een baseline te vestigen, waarna in de latere sprints BERT stapsgewijs wordt gefinetuned en geoptimaliseerd.
\end{itemize}
Op deze manier is elke sprint een directe reactie op de kwantitatieve resultaten van de voorgaande periode.
...
\begin{figure*}[H]
    \centering
    % Verander width naar 0.8 of 0.9 om te testen
    \includegraphics[width=0.85\textwidth]{../voorstel/img/image.png}
    \caption{\label{fig:gantt}Gantt diagram...}
\end{figure*}
